        ...
        <div class="card mb-4 col-md-8">
            ...
            <div class="card-body">
                <form action="{{ route('books.update', $book) }}" method="POST" enctype="multipart/form-data">
                    @method('patch')
                    <x-form-book-create :book="$book" :publishers="$publishers" :authors="$authors" :categories="$categories" />
                            ...
                            <select id="category" class="form-control" name="category">
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ $book->category == $category ? 'selected' : '' }}>{{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('category')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6">
                            <select id="authors" multiple class="form-control" name="authors[]">
                                <option disabled {{ $book->authors()->count() == 0 ? 'selected' : '' }}>اختر المؤلفين
                                </option>
                                @foreach ($authors as $author)
                                    <option value="{{ $author->id }}"
                                        {{ $book->authors->contains($author) ? 'selected' : '' }}>{{ $author->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('authors')
                            ...
                            <select id="publisher" class="form-control" name="publisher">
                                ...
                                @foreach ($publishers as $publisher)
                                    <option value="{{ $publisher->id }}"
                                        {{ $book->publisher == $publisher ? 'selected' : '' }}>{{ $publisher->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('publisher')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6">
                            <textarea id="description" type="text" class="form-control @error('description') is-invalid @enderror"
                                name="description" value="{{ old('description') }}" autocomplete="description">{{ $book->description }}</textarea>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6">
                            <input id="number_of_copies" type="number"
                                ...
                                name="number_of_copies" value="{{ $book->number_of_copies }}"
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6">
                            <input id="price" type="number"
                                ...
                                value="{{ $book->price }}" autocomplete="price">
